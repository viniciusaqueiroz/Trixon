...
const StrokeText = ({
  ...
  const numericFontSize = typeof fontSize === 'number' ? fontSize : 128;
  ...
  const fontStyle = useMemo(
    () => ({
      ...
      fontWeight,
      letterSpacing: toCssLength(letterSpacing),
    }),
  ...
  const strokeTextHeight = style['--stroke-text-height'] || (typeof fontSize === 'number' ? `${Math.round(fontSize * 1.3)}px` : '1em');

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, '--stroke-text-height': strokeTextHeight }}
      role="img"
